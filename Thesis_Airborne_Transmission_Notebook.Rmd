...
Distance_infected_source_office<- Distance_infected_source_office[-c(1,1:3)] 
#If you want to find mean distance and then risk of 1000 random iterations use the below line

#mean_distances_office[i] <- mean(mean_distance_office_test)}
}
...
Cumulitve_risk_office<- as.numeric(lapply(Distance_infected_source_office[ 2:20,3], calcRisk))

Cumulative_risk_office<-Cumulitve_risk_office[-c(1,1:3 )]#Drop infected source

#Add Risk to Dataframe
Distance_infected_source_office$Risk <- Cumulitve_risk_office
#Find cumulative risk and distance
Cumulitve_risk_office<- sum(Cumulitve_risk_office)/19 #people in enclosed space minus infected sources
Cumulitve_distance_office<- mean(Distance_infected_source_office$distance_from_index)
```
...
Risk_plane = Cumalitve_risk_plane # Reassign the static risk measure to the cumulative risk for all susceptible persons 
```
